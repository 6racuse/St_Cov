...
function estimateSharedPopulationNearStore(store, stores, populationPoints, baseRadiusKm) {
  ...
  populationPoints.forEach((point) => {
    ...
    const coversCurrentStore = coveringStores.some(({ store: candidateStore }) => candidateStore.lat === store.lat && candidateStore.lng === store.lng);
    if (!coversCurrentStore) return;
